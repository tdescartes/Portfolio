...
export const ProjectsSection: React.FC<ProjectsSectionProps> = ({
  ...
  const getCategoryIcon = (iconName: string) => {
    switch (iconName) {
      case 'memory':
        return <Cpu className="w-8 h-8 text-[#28e98c]" />;
      case 'smart_toy':
        return <Bot className="w-8 h-8 text-[#28e98c]" />;
      case 'domain':
        return <Building2 className="w-8 h-8 text-[#28e98c]" />;
      case 'code_blocks':
        return <Code2 className="w-8 h-8 text-[#28e98c]" />;
      case 'desktop_windows':
        return <Monitor className="w-8 h-8 text-[#28e98c]" />;
      default:
        return <Layers className="w-8 h-8 text-[#28e98c]" />;
    }
  };
    ...
    <section id="projects" className="bg-[#1A1D23]/30 py-24 border-y border-[#2D3139]">
      <div className="px-4 sm:px-8 md:px-16 max-w-[1200px] mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <h2 className="font-headline text-2xl md:text-3xl font-bold text-[#28e98c] flex items-center gap-4">
            <span className="text-[#2D3139] font-code-label text-xl">03.</span> Featured Work
          </h2>
              ...
              <button
                ...
                className={`font-code-label text-xs px-3 py-1.5 rounded transition-all cursor-pointer ${
                  activeCategory === cat && !selectedSkillFilter
                    ? 'bg-[#28e98c] text-[#000000] font-bold shadow'
        ...
        {selectedSkillFilter && (
          <div className="mb-6 px-4 py-2 bg-[#1A1D23] border border-[#28e98c]/40 rounded flex justify-between items-center text-xs font-code-label">
          ...
          {filteredProjects.map((project) => {
            ...
            return (
              <div
                ...
                } group relative overflow-hidden rounded-xl border border-[#2D3139] bg-[#111317] hover:border-[#28e98c] transition-colors duration-300 cursor-pointer shadow-xl flex flex-col justify-between`}
              >
                ...
                <div className="relative z-20 h-full p-8 flex flex-col justify-between">
                  ...
                  {!isLarge && (
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <button
                        ...
                        className="p-2 text-[#999999] hover:text-[#28e98c] transition-colors"
                  ...
                  <div className={isLarge ? 'max-w-xl my-auto' : 'mt-auto'}>
                    <div className="font-code-label text-xs text-[#28e98c] mb-2 uppercase tracking-wider font-semibold">
                      {project.category}
                    </div>

                    <h3 className="font-headline text-2xl md:text-3xl font-bold mb-3 text-[#28e98c] group-hover:text-[#59ffa3] transition-colors flex items-center justify-between">
                      {project.title}
                      {isLarge && (
                        <ExternalLink className="w-5 h-5 text-[#28e98c] group-hover:translate-x-0.5 transition-transform" />
                      )}
                    </h3>
                    ...
                    <div className="flex flex-wrap gap-3 items-center">
                      {project.tags.map((tag) => (
                        <span
                          key={tag}
                          className="font-code-label text-xs text-[#999999] bg-[#1A1D23]/80 px-2.5 py-1 rounded border border-[#2D3139]"
                        >
                          {tag}
                        </span>
                      ))}

                      <span className="ml-auto font-code-label text-xs text-[#28e98c] opacity-0 group-hover:opacity-100 transition-opacity">
                        Explore System →
                      </span>
                    </div>
                  </div>
